...
class Course(BaseModel):
    ...
    # 真实价格 调用 折扣后的价格 函数，课程列表页
    def real_price(self):
        """计算课程的真实价格"""

        price = self.price
        # 找到当前课程所参与的活动
        return self.get_discount_price(float(price))
    ...
    def real_expire_price(self,expire_id):
        ...
        price = 0
        try:
            expire = self.course_expire.filter(is_show=True, is_delete=False,pk=expire_id)[0]
            price = expire.price
        except:
